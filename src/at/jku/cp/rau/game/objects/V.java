...
public class V implements Comparable<V>, Serializable, ContainsPos {
    ...
    /**
     * although this constructor takes 'int's, internally the values are stored
     * as 'short'
     * 
     * @param x
     * @param y
    ...
    public static V fromString(String s) {
        ...
        if (m.matches()) {
            return new V(Integer.parseInt(m.group(1)), Integer.parseInt(m.group(2)));
        }
    ...
    public static boolean sameLine(V a, V b) {
        return 0 == (Math.abs(a.x) - Math.abs(b.x)) || 0 == (Math.abs(a.y) - Math.abs(b.y));
    }
