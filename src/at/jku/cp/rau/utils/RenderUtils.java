...
public class RenderUtils
{
	...
	public static String visualizePath(IBoard board,
			List<? extends ContainsPos> edges)
	{
	...
	public static String writeBoardAsPNG(String prefix, IBoard board,
			IBoard goal)
	{
		int w = 5;
	...
	public static <T extends Node<T> & ContainsBoard & ContainsMove> void visualizeSearchPath(
			String filename,
			List<T> path)
	{
	...
	{
		return "n" + Integer.toHexString(o.hashCode()) + "_"
				+ ((ContainsBoard) o).getBoard().getTick();
	}

	public static <T extends Node<T> & ContainsBoard & ContainsMove> void visualizeSearchTree(
			String prefix,
			Graph<T> graph,
			List<T> path)
	{
		...
		{
			images.put(nameTick(e.f),
					writeBoardAsPNG(prefix, e.f.getBoard(), goal.getBoard()));
			images.put(nameTick(e.s),
					writeBoardAsPNG(prefix, e.s.getBoard(), goal.getBoard()));
		}

		for (Map.Entry<String, String> entry : images.entrySet())
		{
			dot.add(String.format("%s [image=\"%s\"];",
					entry.getKey(),
					entry.getValue()));
		}

		dot.add(String.format("root -> %s [color=\"magenta\"];",
				nameTick(path.get(0))));

		for (Pair<T, T> e : graph.getEdges())
		{
			String colorcode = Integer.toHexString(e.f.getBoard()
					.getCurrentUnicorn().id == 0 ? hexblue : hexred);
		
			String previous = nameTick(e.f);
			String current = nameTick(e.s);
			
			String taken = pathTest.contains(previous + "_" + current) ? ", penwidth=3"
					: ", style=\"dotted\"";

			dot.add(String.format(
					"%s -> %s [label=\"%s\", color=\"#%s\"%s];",
					nameTick(e.f),
					nameTick(e.s),
					e.s.getMove().toString(),
					colorcode,
					taken));
	...
	public static <T extends Node<T> & ContainsBoard & ContainsMove> void visualizeSearchGraph(
			String prefix,
			Graph<T> graph,
			List<T> path)
	{
		...
		{
			String nodepng = writeBoardAsPNG(prefix, bn.getBoard(),
					goal.getBoard());
			dot.add(String.format("%s [image=\"%s\"];",
					name(bn),
					nodepng));
		...
		for (Pair<T, T> e : graph.getWeightedEdges())
		{
			String colorcode = Integer.toHexString(e.f.getBoard()
					.getCurrentUnicorn().id == 0 ? hexblue : hexred);
			String taken = pathTest.contains(e) ? ", penwidth=3"
					: ", style=\"dotted\"";

			dot.add(String.format(
					"%s -> %s [label=\"%s (%d)\", color=\"#%s\"%s];",
					name(e.f),
					name(e.s),
					e.s.getMove().toString(),
					graph.getWeight(e),
	...
	public static <T extends Node<T>> void visualizeSearchTreeOther(
			String filename,
			Graph<T> graph,
			List<T> path)
	{
		...
		for (T bn : graph.getVertices())
		{
			if (bn.equals(goal))
			{
				dot.add(String.format("\"%s\" [label=\"%s (goal)\"];",
						bn.toString(), bn.toString()));
			}
			else
			{
		...
		for (Pair<T, T> e : graph.getWeightedEdges())
		{
			...
			{
				dot.add(String.format("\"%s\" -> \"%s\" [label=\"(%d)\"];",
						e.f.toString(),
						e.s.toString(),
						graph.getWeight(e)));
			}
		...
		try
		{
			...
			Files.write(Paths.get(filename), dot, StandardCharsets.UTF_8,
					StandardOpenOption.CREATE_NEW);
		} catch (IOException e)
		{
			throw new RuntimeException(e);
