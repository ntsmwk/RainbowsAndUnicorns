...
public class VisualizeExamples {
    public static <T extends Node<T> & ContainsBoard & ContainsMove> void visualize(String dirname, String name,
            IBoard board, Class<T> clazz, Search<GraphTelemetry<T>> searcher) throws Exception {
        Predicate<GraphTelemetry<T>> endReached = new Predicate<GraphTelemetry<T>>() {
            ...
            public boolean isTrueFor(GraphTelemetry<T> current) {
                return ((ContainsBoard) current.content).getBoard().isRunning()
                        && ((ContainsBoard) current.content).getBoard().getUnicorns().get(0).pos
                                .equals(((ContainsBoard) current.content).getBoard().getMarkers().get(0).pos);
        ...
        List<GraphTelemetry<T>> path = searcher.search(new GraphTelemetry<T>(graph, cons.newInstance(board)),
                endReached);
        ...
        RenderUtils.visualizeSearchGraph(dirname + name + "_states", graph, unwrappedPath);

        RenderUtils.visualizeSearchTree(dirname + name + "_tree", graph, unwrappedPath);

        RenderUtils.visualizeSearchPath(dirname + name + "_path", unwrappedPath);
    }

    public static void main(String[] args) throws Exception {
        List<String> lvl = Arrays.asList("#####", "#p..#", "#..m#", "#####");

        List<String> costs = Arrays.asList("#####", "#111#", "#111#", "#####");
        ...
        visualize("/home/rainer/Coding/cp/scratch/graphs/", "bfs", board.copy(), SafeBNode.class,
                new BFS<GraphTelemetry<SafeBNode>>());

        visualize("/home/rainer/Coding/cp/scratch/graphs/", "ids", board.copy(), SafeBNode.class,
                new IDS<GraphTelemetry<SafeBNode>>(100));

        visualize("/home/rainer/Coding/cp/scratch/graphs/", "astar", board.copy(), SafeBNode.class,
                new ASTAR<GraphTelemetry<SafeBNode>>(costfunction, heuristic));

        visualize("/home/rainer/Coding/cp/scratch/graphs/", "random", board.copy(), SafeBNode.class,
